...
def _download_TTE_heasarc(trignum, save_dir=None, match=None, clobber=False):
    tdrssrows = from_heasarc(
        tablename="swifttdrss",
        Target_ID=trignum,
    ...
    for row in tdrssrows:
        ...
        if row["MASTER_FLAG"].strip() == "Y":
            ...
            dirurl = (
                f"https://heasarc.gsfc.nasa.gov/FTP/swift/data/tdrss/{t:%Y_%m}/{obsid}"
            )
            dirpath = datadir(tdrss=True).joinpath(obsid)
            fileurls = dir_from_url(dirurl)
            result[obsid] = dict(
                obsoutdir=dirpath, success=False, downloaded=[], files=[]
            )
